...
Q_LOGGING_CATEGORY(lcAc, "yoru.services.ac", QtInfoMsg)
Q_LOGGING_CATEGORY(lcAcWorker, "yoru.services.ac.worker", QtInfoMsg)

namespace yoru::services {
...
PipeWireWorker::PipeWireWorker(std::stop_token token, AudioCollector* collector)
    ...
    m_stream = pw_stream_new_simple(pw_main_loop_get_loop(m_loop), "yoru-shell", props, &events, this);
    if (!m_stream) {
