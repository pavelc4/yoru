...
MaterialShape { // App icon
    ...
    color: isUrgent ? Colours.palette.m3primaryContainer : Colours.palette.m3secondaryContainer
    Loader {
        ...
        sourceComponent: MaterialSymbol {
            text: {
                const defaultIcon = Utils.findSuitableMaterialSymbol("")
                const guessedIcon = Utils.findSuitableMaterialSymbol(root.summary)
                return (root.urgency == NotificationUrgency.Critical && guessedIcon === defaultIcon) ?
                    "priority_high" : guessedIcon
            }
            anchors.fill: parent
            color: isUrgent ? Colours.palette.m3onPrimaryContainer : Colours.palette.m3onSecondaryContainer
