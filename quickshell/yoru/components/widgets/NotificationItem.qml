...
Item { // Notification item area
    ...
    property real fontSize: Tokens.font.size.small
    ...
    SequentialAnimation { // Drag finish animation
        ...
        NumberAnimation {
            ...
            easing.type: Tokens.anim.expressiveDefaultSpatial.type
            easing.bezierCurve: Tokens.anim.expressiveDefaultSpatial.bezierCurve
        }
        onFinished: () => {
    ...
    NotificationAppIcon { // App icon
        ...
        Behavior on opacity {
            animation: NumberAnimation { duration: Tokens.anim.durations.expressiveFastSpatial; easing: Tokens.anim.expressiveFastSpatial }
        }
    ...
    Rectangle { // Background of notification item
        ...
        Behavior on anchors.leftMargin {
            ...
            NumberAnimation {
                duration: Tokens.anim.durations.expressiveDefaultSpatial
                easing.type: Tokens.anim.expressiveDefaultSpatial.type
                easing.bezierCurve: Tokens.anim.expressiveFastSpatial.bezierCurve
            }
        }

        color: (expanded && !onlyNotification) ? 
            (notificationObject.urgency == NotificationUrgency.Critical) ? 
                Qt.tint(Colours.palette.m3secondaryContainer, Colours.palette.m3surfaceContainer, 0.35) :
                (Colours.palette.m3surfaceContainerHigh) :
            Qt.alpha(Colours.palette.m3surfaceContainerHigh)

        implicitHeight: expanded ? (contentColumn.implicitHeight + padding * 2) : summaryRow.implicitHeight
        Behavior on implicitHeight {
            animation: NumberAnimation { duration: Tokens.anim.durations.expressiveDefaultSpatial; easing: Tokens.anim.expressiveDefaultSpatial }
        }
        ...
        ColumnLayout { // Content column
            ...
            RowLayout { // Summary row
                ...
                StyledText {
                    ...
                    text: {
                        return Utils.processNotificationBody(notificationObject.body, notificationObject.appName || notificationObject.summary).replace(/\n/g, "<br/>")
                    }
                }
            }
                ...
                StyledText { // Notification body (expanded)
                    ...
                    text: {
                        return `<style>img{max-width:${expandedContentColumn.width}px;}</style>` + 
                            `${Utils.processNotificationBody(notificationObject.body, notificationObject.appName || notificationObject.summary).replace(/\n/g, "<br/>")}`
                    }

                    onLinkActivated: (link) => {
                        Qt.openUrlExternally(link)
                        root.qmlParent.modelData.popup = false
                    }
                            ...
                            NotificationActionButton {
                                ...
                                contentItem: MaterialSymbol {
                                    ...
                                    color: (notificationObject.urgency == NotificationUrgency.Critical) ? 
                                        Colours.palette.m3onSurfaceVariant : Colours.palette.m3onSurface
                                    text: "close"
                                }
                            }
